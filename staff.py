...
def add_commands(tree):
	...
	async def addstaff(context, name:str, flavor:str, position:str, order:int):
		...
		if(not result is None):
			...
			con.close()
			return
		cur.execute("INSERT INTO staff(name, flavor, position, order) VALUES(?, ?, ?, ?)", (name, flavor, position, order))
		con.commit()
		con.close()
	...
	async def editpastevent(context, name:str, property:str, value:str):
		if(not await util.check_admin(context)):
			return
		properties = ["name", "flavor", "position", "order"]
